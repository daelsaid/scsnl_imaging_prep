...
function remove_nvols_prt () {
    ...
    for subj_visit_sess in "$@"; do
        ...
        nii_tpmv=$(ls -d ${rawdir}/${subj_visit_sess}/fmri/*PRT_10_tp_mv2_func*/un*/I.nii.gz | grep -v rest | grep -v PRT_09_dm_mv1_func | grep -v pepolar | grep -v calibration);
        for nii_tp in ${nii_tpmv}; do #13 270
            if [ -n "$nii_tp" ]; then  # Ensure the file exists
                echo "Processing: $nii_tp";
                ...
                if [ ! -f "$orig_file" ]; then
                    ...
                    final_vols=$((orig_nframes - 13));
                    if [ "$nframes" -gt "$final_vols" ]; then
                        echo "Removing first 13 volumes from ${taskname}...";
        ...
        nii_dmmv=$(ls -d ${rawdir}/${subj_visit_sess}/fmri/PRT_09_dm_mv1_func/un*/I.nii.gz | grep -v rest | grep -v PRT_10_tp_mv2_func | grep -v pepolar | grep -v calibration);         #13 775
        for nii_dm in ${nii_dmmv}; do
            if [ -n "$nii_dm" ]; then  # Ensure the file exists
                echo "Processing: $nii_dm";
